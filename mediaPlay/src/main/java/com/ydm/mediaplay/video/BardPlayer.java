package com.ydm.mediaplay.video;
...
import com.ydm.mediaplay.R;
import com.ydm.mediaplay.bean.IVideoInfo;
import com.ydm.mediaplay.danmu.DanmuAdapter;
import com.ydm.mediaplay.danmu.DanmuModel;
import com.ydm.mediaplay.danmu.DanmuView;
import com.ydm.mediaplay.listener.OnPlayerCallback;
import com.ydm.mediaplay.listener.OnVideoControlListener;
import com.ydm.mediaplay.utils.NetworkUtils;
import com.ydm.mediaplay.view.BardVideoControllerView;
import com.ydm.mediaplay.view.BardVideoProgressOverlay;
import com.ydm.mediaplay.view.BardVideoSystemOverlay;
import com.ydm.mediaplay.view.VideoBehaviorView;

/**
 * Description:视频播放器核心使用SurfaceView + MediaPlayer封装，其中MediaPlayer可以轻松更换成三方MediaPlayer
 ...
 * Data：2019/1/30-14:38
 * Author: DerMing_You
 */
public class BardPlayer extends VideoBehaviorView {
    private static final String TAG = "JsPlayer";
    private SurfaceView surfaceView;
    private View loadingView;
    private BardVideoProgressOverlay progressView;
    private BardVideoSystemOverlay systemView;
    private BardVideoControllerView mediaController;
    // 弹幕
    private DanmuView danmu;

    private BardMediaPlayer mMediaPlayer;
    ...
    public BardPlayer(Context context) {
        super(context);
        init();
    }

    public BardPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public BardPlayer(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        ...
        progressView = (BardVideoProgressOverlay) findViewById(R.id.video_progress_overlay);
        systemView = (BardVideoSystemOverlay) findViewById(R.id.video_system_overlay);
        mediaController= (BardVideoControllerView) findViewById(R.id.video_controller);
    ...
    private void initPlayer() {
        mMediaPlayer = new BardMediaPlayer();
        ...
        mMediaPlayer.setOnPlayerListener(new OnPlayerCallback() {
            ...
            public void onStateChanged(int curState) {
                switch (curState) {
                    case BardMediaPlayer.STATE_IDLE:
                        am.abandonAudioFocus(null);
                        break;
                    case BardMediaPlayer.STATE_PREPARING:
    ...
    protected void updateVolumeUI(int max, int progress) {
        systemView.show(BardVideoSystemOverlay.SystemType.VOLUME, max, progress);
    }

    @Override
    protected void updateLightUI(int max, int progress) {
        systemView.show(BardVideoSystemOverlay.SystemType.BRIGHTNESS, max, progress);
    }
    ...
    public BardVideoControllerView getMediaController() {
        return mediaController;
    }

    public void setMediaController(BardVideoControllerView mediaController) {

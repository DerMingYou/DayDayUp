package com.ydm.mediaplay.view;
...
import com.ydm.mediaplay.listener.OnBardVideoControlListener;
...
import com.ydm.mediaplay.utils.StringUtils;
import com.ydm.mediaplay.video.BardMediaPlayer;

/**
 * Description: 视频控制器，可替换或自定义样式
 * Data：2019/1/30-14:46
 * Author: DerMing_You
 */
public class BardVideoControllerView extends FrameLayout {
    ...
    private BardVideoErrorView mErrorView;
    ...
    private BardMediaPlayer mPlayer;
    ...
    public BardVideoControllerView(Context context) {
        super(context);
        init();
    }

    public BardVideoControllerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public BardVideoControllerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initControllerPanel() {
        ...
        // error
        mErrorView = (BardVideoErrorView) findViewById(R.id.video_controller_error);
        mErrorView.setOnVideoControlListener(new OnBardVideoControlListener() {
            @Override
     ...
     * @param player
     */
    public void setMediaPlayer(BardMediaPlayer player) {
        mPlayer = player;
        updatePausePlay();
    }
    ...
    public void checkShowError(boolean isNetChanged) {
        ...
        if (isConnect) {
            // 如果已经联网
            if (mErrorView.getCurStatus() == BardVideoErrorView.STATUS_NO_NETWORK_ERROR && !(isMobileNet && !isWifiNet)) {
                // 如果之前是无网络
                mErrorView.setVisibility(View.GONE);
            } else if (videoInfo == null) {
                // 优先判断是否有video数据
                showError(BardVideoErrorView.STATUS_VIDEO_DETAIL_ERROR);
            } else if (isMobileNet && !isWifiNet && !mAllowUnWifiPlay) {
                // 如果是手机流量，且未同意过播放，且非本地视频，则提示错误
                mErrorView.showError(BardVideoErrorView.STATUS_UN_WIFI_ERROR);
                mPlayer.pause();
            } else if (isWifiNet && isNetChanged && mErrorView.getCurStatus() == BardVideoErrorView.STATUS_UN_WIFI_ERROR) {
                // 如果是wifi流量，且之前是非wifi错误，则恢复播放
                playFromUnWifiError();
            } else if (!isNetChanged) {
                showError(BardVideoErrorView.STATUS_VIDEO_SRC_ERROR);
            }
        } else {
            // 无网，暂停播放并提示
            mPlayer.pause();
            showError(BardVideoErrorView.STATUS_NO_NETWORK_ERROR);
        }
    }
    ...
    private void retry(int status) {
        ...
        switch (status) {
            case BardVideoErrorView.STATUS_VIDEO_DETAIL_ERROR:
                // 传递给activity
                if (onVideoControlListener != null) {
                    onVideoControlListener.onRetry(status);
                }
                break;
            case BardVideoErrorView.STATUS_VIDEO_SRC_ERROR:
                reload();
                break;
            case BardVideoErrorView.STATUS_UN_WIFI_ERROR:
                allowUnWifiPlay();
                break;
            case BardVideoErrorView.STATUS_NO_NETWORK_ERROR:
                // 无网络时
                if (NetworkUtils.isNetworkConnected(getContext())) {
                    if (videoInfo == null) {
                        // 如果video为空，重新请求详情
                        retry(BardVideoErrorView.STATUS_VIDEO_DETAIL_ERROR);
